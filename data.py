...
def TrainNN(num_hiddens, eps, momentum, num_epochs, C = 0):
  ...
  for epoch in xrange(num_epochs):
    ...
    train_MCE = incorrect(prediction, target_train)
    ...
    train_MCE_error.append(train_MCE)
    valid_MCE_error.append(valid_MCE)
    if C == 1:
...
def incorrect(prediction, target):
  incorrect = 0
  ...
  return (float(incorrect) / float(target.size)) * 100
